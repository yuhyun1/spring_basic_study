...
> 아래 그림처럼 세 명의 고객이 요청하면 3개의 객체가 생성된다.
> 그러나 세 명이 아니라 만 명의 고객이 요청한다면 10000개의 객체가 생성될 것이고, 그에 따른 메모리 소비도 엄청날 것이다.
...
_참고: 싱글톤 패턴을 구현하는 방법은 여러가지가 있다. 여기서는 객체를 미리 생성해두는 가장 단순하고
안전한 방법을 선택했다._
...
> 요약 : 싱글톤의 기본적인 이해가 목적. 이렇게 사용하지 않고, 어차피 스프링 컨테이너가 기본적으로 싱글톤 패턴으로 객체를 관리해 줌.
...
_참고: 스프링의 기본 빈 등록 방식은 싱글톤이지만, 싱글톤 방식만 지원하는 것은 아니다. 요청할 때 마다
새로운 객체를 생성해서 반환하는 기능도 제공한다. 자세한 내용은 뒤에 빈 스코프에서 설명함.
*99%는 싱글톤으로 사용함._
> 요약 : 스프링은 기본적으로 싱글톤 방식으로 동작한다.
...
#### 상태를 유지할 경우 발생하는 문제점 예시 #2 (테스트 코드)
```java
...
- ThreadA가 사용자A 코드를 호출하고 ThreadB가 사용자B 코드를 호출한다 가정하자. (웹에서 요청이 올 때, 각각 쓰레드가 따로 할당됨.)
- StatefulService 의 price 필드는 공유되는 필드인데, 특정 클라이언트가 값을 변경한다. (위에서 무상태 설계 2번)
- 사용자A의 주문금액은 10000원이 되어야 하는데, 20000원이라는 결과가 나왔다.
- 실무에서 이런 경우를 종종 보는데, 이로인해 정말 해결하기 어려운 큰 문제들이 터진다.(몇년에 한번씩 꼭
  만난다.)
- 진짜 공유필드는 조심해야 한다! 스프링 빈은 항상 무상태(stateless)로 설계하자.

#### 수정된 코드 (간단히) => 무상태로 설계하는 법 
```java
public class StatefulService {
...
```
> 요약 : 지역변수, 파라미터, ThreadLocal 등을 이용하여 무상태로 설계해야 한다.
> 이번 강의에서는 깊게 다루지 않은 듯함.
...
```
- 테스트를 위해 MemberRepository를 조회할 수 있는 기능을 추가한다. 기능 검증을 위해 잠깐 사용하는
...
```
> 요약 : 스프링이 어떠한 방법으로 싱글톤을 보장하고 있음을 이해.
...
클래스를 상속받은 임의의 다른 클래스를 만들고, 그 다른 클래스(밑에 그림의 AppConfig@CGLIB)를 스프링 빈으로 등록한 것이다!
#### 그림으로 간단히 이해
![img.png](img/img13.png)
이름은 appConfig 지만, 인스턴스 객체가 AppConfig@CGLIB 가 들어가있다.
...
_참고 : AppConfig@CGLIB는 AppConfig의 자식 타입이므로, AppConfig 타입으로 조회 할 수 있다._
...
- @Bean만 사용해도 스프링 빈으로 등록되지만, 싱글톤을 보장하지 않는다.
  - @Configuration이 없으면 memberRepository() 처럼 의존관계 주입이 필요해서 메서드를 직접 호출할 때 싱글톤을 보장하지
않는다.
- 크게 고민할 것이 없다. 스프링 설정 정보는 항상 @Configuration 을 사용하자.
> 요약 : 스프링이 내가 만든 코드를 그대로 상속받은 다른 클래스를 만들어서 스프링 빈으로 등록했고,
> 그 클래스에서 이미 스프링 빈이 존재하면 존재하는 빈을 반환하고, 스프링 빈이 없으면 생성해서 스프링 빈으로 등록하고 반환하는 코드를 넣어서
> 싱글톤을 보장한 것이다. 그리고 
